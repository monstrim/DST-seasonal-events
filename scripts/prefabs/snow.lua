-- replicated and adapted from prefabs/snow
...
local function fn()
    ...
    local function update_fn()
        if init_effect then
            ...
            -- pretty snow
            effect:SetRenderResources(1, WINTER_TEXTURE, SHADER)
            effect:SetScaleEnvelope(1, WINTER_SCALE_ENVELOPE_NAME)
            effect:SetUVFrameSize(1, .125, 1)
            effect:SetRotationStatus(1, true)
            effect:SetAcceleration(1, -1, -9.80, 1)
            effect:SetDragCoefficient(1, .85)
            effect:SetMaxNumParticles(1, 4800)
            ...
            effect:SetSortOrder(1, 3)
            effect:EnableDepthTest(1, true)
        end

        pretty_snow = IsSpecialEventActive(SPECIAL_EVENTS.WINTERS_FEAST)
